...
| `Hero_Hit_BladeFlesh_SFX.wav` | 0.400 秒 | 确认造成伤害时播放；采用 H-B，强化硬切入肉、湿润爆点和紧实低频，不包含挥砍破风声 |
| `Hero_Kill_Confirm_SFX.wav` | 0.620 秒 | 击杀确认时使用独立 AudioSource 播放并按连杀数升调 |
| `Hero_KillChainEnd_Sheathe_SFX.wav` | 0.680 秒 | 连杀真正结束时播放；采用 S-D，突出光滑入鞘、明亮金属脆鸣和坚实锁鞘声，无锈蚀拖刮感 |
...
3. 普通挥砍（砍空气）只播放现有攻击破风声；`Hero_Hit_BladeFlesh_SFX.wav` 不包含挥砍声，仅在伤害真正确认后额外播放。冲刺风切声可与攻击动作声叠加。
...
- 初始会话：`minigame_wuxia_sfx_v2`
- 本次选定会话：`minigame_wuxia_sfx_v5_candidates`
- 完美闪避：`kling_0affa24c4391`
- 子弹时间：`kling_8147a22f3f67`
- 冲刺：`kling_aef378dbffa9`
- 命中（H-B）：`kling_1d702ddfcd1b`
- 击杀：`kling_8552b8ed3cae`
- 连杀结束（S-D）：`kling_cb8224b0c5c4`
